...
description: >-
  ...
  polling, and explicit downloads when requested.
...
文生图、图生说话视频 训练与轮询；用户明确要求时下载生成物。凭据与权限见 **`manifest.yaml`**。脚本**不**依赖 ffmpeg/ffprobe。
...
| `list_tasks.py`         | 按 `--kind=photo|motion` 分页列出任务 |
